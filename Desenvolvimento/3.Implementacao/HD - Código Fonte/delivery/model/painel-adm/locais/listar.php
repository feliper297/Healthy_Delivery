...
<table class="table table-sm mt-3 tabelas">
	...
	if($txtbuscar == ''){
		$res = $pdo->prepare("SELECT * from locais order by nome asc LIMIT $limite, $itens_por_pagina");
	}else{
		$txtbuscar = '%'.@$_POST['txtbuscar'].'%';
		$res = $pdo->prepare("SELECT * from locais where nome LIKE '$txtbuscar' order by nome asc");
	...
	$dados = $res->fetchAll(PDO::FETCH_ASSOC);
		...
		$res_todos = $pdo->prepare("SELECT * from locais");
